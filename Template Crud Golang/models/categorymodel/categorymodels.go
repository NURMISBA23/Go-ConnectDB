...
func GetAll() []entities.Categoriess {
	...
	var categories []entities.Categoriess

	// Loop through the rows
	for rows.Next() {
		var category entities.Categoriess

		// Scan the data from the rows and store it in the struct
		if err := rows.Scan(&category.Id, &category.Name, &category.Status, &category.CreatedAt, &category.UpdatedAt); err != nil {
			panic(err)
		}
...
func Create(category entities.Categoriess) bool {

	// Insert the data into the table
	// The data is stored in the struct
	result, err := config.DB.Exec(`
		INSERT INTO categoriess (name, created_at, update_at) 
		VALUE (?, ?, ?)`,
		category.Name,
		category.CreatedAt,
		category.UpdatedAt,
...
func Detail(id int) entities.Categoriess {
...
func Update(id int, category entities.Categoriess) bool {

	// Update the data in the table
	query, err := config.DB.Exec("UPDATE categoriess SET name = ?, status = ? , created_at = ?,update_at = ? where id = ?", category.Name, category.Status, category.CreatedAt, category.UpdatedAt, id)
